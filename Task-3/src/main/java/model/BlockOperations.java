...
public class BlockOperations {
    ...
    public static double[][] reconstructMatrix(List<Block> blocks){
        ...
        Integer currentBlockInd = 0;
        for(int ind = 0; ind <blocks.get(0).getOriginalHeight(); ind += 8)
            for( int ind2=0; ind2<blocks.get(0).getOriginalWidth(); ind2 += 8){
                Block currentBlock = blocks.get(currentBlockInd);
                for(int i=0; i<8; i++)
                    for(int j=0; j<8; j++)
                        elements[ind+i][ind2+j] = currentBlock.getBlock()[i][j];
                currentBlockInd += 1;
            }
        return elements;
    }
    ...
    public static void fromGtoBlock(Block b){
        double[][] block = new double[8][8];
        for(int i=0; i<8; i++)
            for(int j=0; j<8; j++)
                block[i][j] = (double) b.getGMatrix()[i][j];
        b.setBlock(block);
    }
}
